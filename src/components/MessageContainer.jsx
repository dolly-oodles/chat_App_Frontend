...
const MessageContainer = ({ onBackUser }) => {
  ...
  useEffect(() => {
    const getMessages = async () => {
      ...
      try {
        ...
        const msg = await axiosInstance.get(`message/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const sendMsg = await axiosInstance.post(`message/send/${id}`, {
        message: sendData,
      });
            ...
            <div className="w-full rounded-full flex items-center bg-white">
              ...
              <button type="submit">
                {loading ? (
                  ...
                  <IoSend
                    size={25}
                    className="text-sky-600 -mx-1 cursor-pointer rounded-full bg-black w-10 h-auto p-2"
                  />
                )}
              </button>
            </div>
